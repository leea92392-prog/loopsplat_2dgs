...
class GaussianViewer(Viewer):
    ...
    def import_server_modules(self):
        ...
        def _draw_poses_stub(image, viewmatrix, fov_x, pose_sizes, width, height, Rts, f, color):
            """Stub when draw_poses is not available (LoopSplat mode)."""
            return image

        def _draw_anchors_stub(image, viewmatrix, fov_x, anchor_sizes, anchors, anchor_weights):
            """Stub when draw_anchors is not available (LoopSplat mode)."""
            return image

        try:
            from utils import draw_poses, draw_anchors
        except ImportError:
            draw_poses = _draw_poses_stub
            draw_anchors = _draw_anchors_stub

    @classmethod
    def from_output_dir(cls, output_dir: str, mode: ViewerMode, args: Namespace = None):
    ...
    @classmethod
    def from_scene_model(cls, scene_model, mode: ViewerMode):
        viewer = cls(mode)
    ...
    def step(self):
        ...
        if self.ellipsoid_viewer.num_gaussians != self.scene_model.n_active_gaussians:
            scales = (self.scene_model.get_scaling_for_ellipsoid()
                     if hasattr(self.scene_model, 'get_scaling_for_ellipsoid')
                     else self.scene_model.scaling.detach().cpu().numpy())
            colors = (self.scene_model.get_colors_for_ellipsoid()
                     if hasattr(self.scene_model, 'get_colors_for_ellipsoid')
                     else self.scene_model.f_dc.detach().cpu().numpy())
            if colors.ndim == 3 and colors.shape[1] == 1:
                n = colors.shape[0]
                c48 = np.zeros((n, 48), dtype=np.float32)
                c48[:, 0] = colors[:, 0, 0]
                c48[:, 1] = colors[:, 0, 1]
                c48[:, 2] = colors[:, 0, 2]
                colors = c48
            if scales.shape[1] == 2:
                s3 = np.minimum(scales[:, 0], scales[:, 1])
                scales = np.concatenate([scales, s3[:, None]], axis=1)
    ...
    def show_gui(self):
        ...
        with imgui_ctx.begin("Point View"):
            ...
            # Overlay invisible button so view area is focusable; imgui.image() cannot receive focus
            # and keys would be captured by Settings panel controls (e.g. ellipsoid scaling_modifier)
            view_min = imgui.get_item_rect_min()
            view_size = imgui.get_item_rect_size()
            imgui.set_cursor_screen_pos(view_min)
            imgui.invisible_button("point_view_capture##point", view_size)
        ...
        if self.show_top_view:
            with imgui_ctx.begin("Top View"):
                ...
                view_size = imgui.get_item_rect_size()
                imgui.set_cursor_screen_pos(view_min)
                imgui.invisible_button("point_view_capture##top", view_size)
                if imgui.is_item_hovered():
                    imgui.set_keyboard_focus_here()
                if imgui.is_item_hovered():
...
if __name__ == "__main__":
    ...
    if mode is ViewerMode.CLIENT:
        ...
        output_dir = getattr(args, "output_dir", None) or getattr(args, "scene_dir", None)
        viewer = GaussianViewer.from_output_dir(output_dir, mode, args)
